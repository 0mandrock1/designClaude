...
const articles: Row[] = [
  ['Debloat API: desk','Коли робоча машина обростає зайвим — зібрати легкий, повторюваний стек.','/tools/debloat-api-desk/',Terminal],
  ['Debloat API: home','Як зробити домашній вузол тихим, зрозумілим і придатним для підтримки.','/tools/debloat-api-home/',Terminal],
  ['Debloat API: media','Як відокремити медіавузол, контролювати шум і не зламати основну систему.','/tools/debloat-api-media/',Terminal],
  ['Cobalt tools','Що поставити навколо медіа‑задач, щоб не збирати один і той самий набір щоразу.','/tools/cobalt-tools/',Braces],
  ['Claude memory system','Коли контекст починає губитися — як перетворити памʼять агента на архітектуру.','/claude/claude-memory-system/',Cpu],
  ['Service block preferences','Як не перетворити сервісні блоки на безформний набір перемикачів.','/claude/service-block-preferences/',Cpu],
  ['Lock/public pipeline','Як розділити публічне й закрите без двох паралельних реальностей.','/claude/lock-public-pipeline/',Cpu],
  ['Shove replay','Як повернути агентну сесію до стану, який можна повторити й розібрати.','/claude/shove-replay/',Cpu],
  ['Audio downloader','Як зібрати аудіо через CLI без ручного бігання між сервісами.','/music/audio-downloader/',Disc3],
  ['Cables.gl','Як пройти від нодового графа до робочого експорту, а не лишитися в демці.','/lessons/cables-gl/',Waves],
  ['Claude Code best practices','Практики для тих, кому потрібно тримати агентну роботу повторюваною й безпечнішою.','/claude/claude-code-best-practices/',Cpu,'LOCKED'],
  ['Claude in Chrome','Як використовувати агента в браузері, коли контекст живе не лише в коді.','/claude/claude-in-chrome/',Cpu,'LOCKED'],
  ['Context rot','Що робити, коли довга сесія деградує і починає втрачати прийняті рішення.','/claude/context-rot/',Cpu,'LOCKED'],
  ['Cowork agents','Робочі ролі й межі для кількох агентів у спільному процесі.','/claude/cowork-agents/',Cpu,'LOCKED'],
  ['Cowork desktop','Нотатки про десктопний агентний процес і його практичні межі.','/claude/cowork-desktop/',Cpu,'LOCKED'],
  ['Prompting checklist','Коротка перевірка перед запуском складного завдання агенту.','/claude/prompting-checklist/',Cpu,'LOCKED'],
  ['Skill triggering','Як саме визначати, коли навичка має включатися в робочий процес.','/claude/skill-triggering/',Cpu,'LOCKED'],
]

function Cards({ rows, kind = 'OPEN', context = false }: { rows: Row[], kind?: string, context?: boolean }) {
  return <div className="grid">{rows.map((x,i)=>{const I=x[3], external=x[2].startsWith('http'), locked=x[4] === 'LOCKED';return <a className="node" href={x[2]} key={x[0]} data-locked={locked || undefined} target={external ? '_blank' : undefined} rel={external ? 'noreferrer' : undefined}><span className="num">{String(i+1).padStart(2,'0')}</span><I size={22}/><div>{context&&<span className="context-label">КОНТЕКСТ / НАВІЩО ЧИТАТИ</span>}<strong>{x[0]}</strong><p>{x[1]}</p><span className="node-pitch">{locked ? 'REQUEST ACCESS' : kind} →</span></div>{x[4]&&<b className={locked ? 'locked-badge' : undefined}>{locked ? '🔒 LOCKED' : x[4]}</b>}<ArrowUpRight className="arrow" size={18}/></a>})}</div>
}
...
export default function App(){
  ...
  return <div className="site" id="top">
    ...
    <main>
      ...
      <section className="articles" id="articles"><Debris seed="articles" name="read" count={50}/><div className="sectionhead"><span>03 / ARTICLES / CONTEXT</span><span>{articles.length} TEXTS</span></div><p className="pretext">Перед кожною статтею є короткий контекст: для кого вона, яку конкретну проблему розбирає і що стане простішим після читання. Закриті матеріали позначені замком до кліку.</p><div className="article-groups"><div><h3>TOOLS / SYSTEMS</h3><Cards rows={articles.slice(0,4)} kind="READ ARTICLE" context/></div><div><h3>CLAUDE / AGENTS</h3><Cards rows={articles.slice(4,15)} kind="READ ARTICLE" context/></div><div><h3>MUSIC / LEARNING</h3><Cards rows={articles.slice(15)} kind="READ ARTICLE" context/></div></div></section>
